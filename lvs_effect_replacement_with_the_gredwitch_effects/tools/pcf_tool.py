...
What `build` does, per gred_tracers_<color>_<caliber> definition:
  ...
  * appends "Remap Control Point to Vector" writing control point 1's
    POSITION straight into the particle's PREV_XYZ (identity remap, output
    field 2). Source particles carry velocity implicitly as
    (xyz - prev_xyz) / m_flPreviousDt, and on the emission frame
    m_flPreviousDt is the fixed 0.05 s that SimulateFirstFrame seeds
    (particles.cpp: UpdatePrevControlPoints(0.05f)), so the Lua side sets
    CP1 = launch position - velocity * 0.05 and the particle leaves at
    exactly the LVS round's velocity. (Reading the engine source ruled out
    the orientation-vector route: GMod normalises it.)
...
# Identity remap window for CP1 (world position minus velocity * 0.05):
# covers any map coordinate plus 5 s of the fastest LVS rounds.
REMAP_RANGE = 1.0e7


def build(src_path, remap_src, out_path):
    ...
    remap_tpl = find_operator(definitions(Reader(open(remap_src, "rb").read())), "Remap Control Point to Vector")
    if lifetime_tpl is None or remap_tpl is None:
    ...
    for d in tracers:
        for suffix, gravity in (("", (0.0, 0.0, -1200.0)), ("_flat", (0.0, 0.0, 0.0))):
            ...
            inits.append(make_operator(remap_tpl, {
                "input control point number": 1,
                "input minimum": (-REMAP_RANGE, -REMAP_RANGE, -REMAP_RANGE),
                "input maximum": (REMAP_RANGE, REMAP_RANGE, REMAP_RANGE),
                "output field": 2,
                "output minimum": (-REMAP_RANGE, -REMAP_RANGE, -REMAP_RANGE),
                "output maximum": (REMAP_RANGE, REMAP_RANGE, REMAP_RANGE),
                "output is scalar of initial random range": 0,
                "offset position": 0,
                "accelerate position": 0,
                "local space CP": -1,
                "emitter lifetime start time (seconds)": -1.0,
                "emitter lifetime end time (seconds)": -1.0,
            }))
