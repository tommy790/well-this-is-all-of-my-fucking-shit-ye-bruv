...
**Fixed**
- Multi-turret vehicles (T-35): the frozen reference model only copied the main turret's pose, so a traversing secondary turret or MG drifted away from its attachment on the reference and the flash slid off the gun (offset growing shot by shot until the resolver gave up). Every pose parameter is now copied, mapped through its range.
