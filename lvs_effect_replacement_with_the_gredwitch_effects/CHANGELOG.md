...
- Flash beside the barrel while a fast mount (Willys pintle MG) is swung: the server and client smooth the mount's aim independently, so the server's shot origin sits a few units off the barrel the client renders during a swing. The applied offset is now the median of the gun's recent shots, which keeps a genuine constant offset (BMD-4M 30mm) and discards the transient swing error.
