...
**Changed**
- Tracers now use Gredwitch's ballistic shell tracer: the same glow-sprite trail Gredwitch draws behind its own shells is emitted behind LVS's bullet every frame, so it drops with gravity on ballistic guns and travels at the weapon's real projectile speed instead of gred's fixed-speed straight beam. Players without this addon still receive the plain straight gred beam from the server.
