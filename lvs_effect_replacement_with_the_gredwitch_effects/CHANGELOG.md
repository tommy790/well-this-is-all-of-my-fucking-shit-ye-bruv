...
**Changed**
- Tracers now follow LVS ballistics: the Gredwitch tracer streak (its own ins_tracer particle image, colours, width and trail length) is emitted every frame at LVS's bullet the way Gredwitch draws the tracer of its own ballistic shells, so it drops with gravity on ballistic guns and travels at the weapon's real projectile speed instead of gred's fixed-speed straight beam. `lvs_gred_fx_tracer_flip 1` reverses the streak if its bright end appears to trail. Players without this addon still receive the plain straight gred beam from the server.
