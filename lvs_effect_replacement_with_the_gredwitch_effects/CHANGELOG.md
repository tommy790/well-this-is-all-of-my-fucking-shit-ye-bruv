...
**Fixed**
- Muzzle attachment resolution is now weapon-code first in structure, not just in order: when the vehicle's weapon code names where the gun fires from, that attachment is used (the barrel whose line passes through the shot on multi-barrel mounts, else the nearest named point); the geometric heuristics run only for vehicles whose code names nothing. The one exception kept is a mount whose code names a shared point and offsets each shot onto a barrel that has its own muzzle attachment on the shot's barrel line (Pz.IV Zerstörer quad and Flakpanzer 341 twin: code names "aim", each shot flashes from its muzzle_N — also when the aim point happens to sit on the bore axis behind the barrels).
